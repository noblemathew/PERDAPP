...
def settings_file():
    """One small JSON in the user's home folder - rotation counters and
    the last save location. Deleting it just resets both."""
    try:
        home = os.path.expanduser("~")
    except Exception:
        home = "."
    return os.path.join(home, ".interview_assistant.json")
...
def read_rotation():
    return {k: v for k, v in read_settings().items() if isinstance(v, int)}
...
def fit_wrap(container, pairs):
    """Keep wrapped labels as wide as the card, whatever the window size."""
    def _resize(event):
        for label, inset in pairs:
            try:
                width = max(300, event.width - inset)
                if abs(int(label.cget("wraplength")) - width) > 6:
...
def default_output_dir():
    """Last folder used, if it is still there. Otherwise Desktop."""
    saved = read_settings().get("output_dir")
    if isinstance(saved, str) and saved.strip():
        parent = os.path.dirname(saved.rstrip(os.sep)) or saved
        if os.path.isdir(saved) or os.path.isdir(parent):
            return saved
...
class App(tk.Tk):
    ...
    def _render_question(self):
        ...
        qlbl = tk.Label(c, text=q["question"], bg=SURFACE, fg=INK,
                        font=font(16, "bold", "display"), wraplength=900,
                        justify="center")
        qlbl.pack(fill="x", pady=(18, 18))

        exp = tk.Frame(c, bg="#F4F7F8")
        exp.pack(fill="x", pady=(0, 14))
        tk.Frame(exp, bg=ACCENT, height=3).pack(fill="x")
        inner = tk.Frame(exp, bg="#F4F7F8", padx=16, pady=13)
        inner.pack(fill="both", expand=True)
        tk.Label(inner, text="EXPECTED ANSWER  \u00b7  INTERVIEWER ONLY", bg="#F4F7F8",
                 fg=MUTED, font=font(8, "bold")).pack(fill="x", pady=(0, 6))
        elbl = tk.Label(inner, text=q["expected"], bg="#F4F7F8", fg=INK, font=font(10.5),
                        wraplength=860, justify="center")
        elbl.pack(fill="x")
        fit_wrap(c, [(qlbl, 30), (elbl, 70)])
    ...
    def show_wrapup(self):
        ...
        for t in self.data["topics"]:
            ...
            for i, q in enumerate(qs):
                ...
                ql = tk.Label(line, text=q["question"], bg=SURFACE, fg=INK,
                              font=font(9.5), anchor="w", wraplength=740,
                              justify="left")
                ql.pack(side="left", fill="x")
                fit_wrap(line, [(ql, 130)])
    ...
    def show_export(self):
        ...
        tk.Label(page, text="Generate the record", bg=CANVAS, fg=INK,
                 font=font(20, "bold", "display")).pack(anchor="w")
        tk.Label(page, text="Two files are written: a plain .txt transcript for the "
                            "agent, and a formatted PDF report.",
                 bg=CANVAS, fg=MUTED, font=font(10)).pack(anchor="w", pady=(4, 20))
        ...
        tk.Label(c, text="A folder named %s is created here. Whatever you choose is "
                         "remembered and used for the next candidate."
                         % self.data["interview_id"],
    ...
    def _build_interview_transcript(self):
        """Plain .txt: the questions, the answers, and the mark for each."""
        cand = self.data["candidate"]
